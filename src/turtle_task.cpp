    ...
    class FG_eval
    {
        public:
            ...
            void operator()(ADvector& fg, const ADvector X)
            {
                ...
                // 2*N constraints for ensuring that the consecutive velocity and anglular velocity do not change by much,
                // 3 more constraints ensuring that the position and the angle remain in valid range.
                assert(X.size() == 2*N);
                // N velocity and N anglular velocity values to be found. (for the next N steps)
                // We need to find these 2*N values

                // Fortran style indexing             
                AD<double> vel[N]; // array storing the next N velocities.
                AD<double> ang_vel[N]; // array storing the next N angular velocities.
...
int main(int argc, char **argv)
{
    ...
    param_file.open("catkin_ws/src/task_turtle_sim/src/params/params.txt");
    ...
    if(param_file.is_open())
    {
        ...
        assert(marker2 == 5); // to ensure that the required number of parameters are given.
    }

    typedef CPPAD_TESTVECTOR( double ) Dvector;
    size_t nx = 2*(fg_eval.N);
    Dvector X_initial(nx);  // initial values of the velocities and the angular velocities. This is required for the optimization algo used by CPPAD
    Dvector X_lower(nx); // lower limits of the velocity and the anglular velocity.
    Dvector X_upper(nx); // upper limits of the velocity and the anglular velocity.
    ...
    gl[2*fg_eval.N+3] = -3.14; gu[2*fg_eval.N+3] = 3.14;
